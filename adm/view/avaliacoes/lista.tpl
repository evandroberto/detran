...
<section id="contact" class="section-padding contact-section bg-light">
	<div class="container">
		...
		<div class="cta-button text-right">
			<a href="{$AVAL_CADASTRO}" class="button">Nova</a>
		</div>
		...
		<table class="table table-hover text-center">
			...
			<tbody>
				...
				<tr>
					...
					<td>{$A.curso}</td>
					<td>
						<a href="{$AVAL_INFO}/{$A.id}/{$A.slug}" title="Detalhar">
							<i class="far fa-eye fa-lg"></i></a>&emsp;

						<form id="edicao{$A.id}" name="edicao" method="post" action="{$AVAL_EDICAO}" style="display:inline;">
							<input type="hidden" name="id" value="{$A.id}">

							<a href="javascript:{}" title="Editar" onclick="document.getElementById('edicao{$A.id}').submit(); return false;"><i class="far fa-edit fa-lg"></i>&emsp;</a>
						</form>

						<form id="exclusao{$A.id}" name="exclusao" method="post" action="{$AVAL_EXCLUSAO}" style="display:inline;">
							<input type="hidden" name="id" value="{$A.id}">

							<a href="javascript:void(0)" title="Excluir" id="excluir" data-id="{$A.id}"><i class="far fa-trash-alt fa-lg"></i>&emsp;</a>
						</form>
					</td>
				</tr>
				{/foreach}
