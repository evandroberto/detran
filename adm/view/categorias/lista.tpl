...
<section class="section-padding contact-section bg-light">
	<div class="container">
		...
		<div class="cta-button text-right">
			<a href="{$CAT_CADASTRO}" class="button">Nova</a>
		</div>
		...
		<table class="table table-hover text-center">
			...
			<tbody>
				...
				<tr>
					...
					<td>{$C.nome}</td>
					<td>
						<a href="{$CAT_INFO}/{$C.id}/{$C.slug}" title="Detalhar">
							<i class="far fa-eye fa-lg"></i></a>&emsp;

						<form id="edicao{$C.id}" name="edicao" method="post" action="{$CAT_EDICAO}" style="display:inline;">
							<input type="hidden" name="id" value="{$C.id}">

							<a href="javascript:{}" title="Editar" onclick="document.getElementById('edicao{$C.id}').submit(); return false;"><i class="far fa-edit fa-lg"></i>&emsp;</a>
						</form>

						<form id="exclusao{$C.id}" name="exclusao" method="post" action="{$CAT_EXCLUSAO}" style="display:inline;">
							<input type="hidden" name="id" value="{$C.id}">
