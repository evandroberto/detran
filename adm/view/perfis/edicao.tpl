				...
				<form action="" method="post" class="contact-form bg-white">
					...
					<div class="row">
						<div class="col-lg-2 form-group">
							<h6>Código</h6>
							<input type="text" class="form-control" name="codigo" placeholder="Código" value="{$P.codigo}" autocomplete="off" required>
						</div>
						<div class="col-lg-10 form-group">
							<h6>Nome</h6>
							<input type="text" class="form-control" name="nome" placeholder="Nome" value="{$P.nome}" autocomplete="off" required>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<select class="form-control" name="curso_id" required>
							...
							{foreach from=$CURSOS item=C}
								<option value="{$C.id}" {if $P.curso_id == $C.id} selected {/if}>{$C.nome}</option>
							{/foreach}
						</select>
					</div>
					...
					<div class="form-btn text-center">
						<a href="{$PAG_PERFIS}" class="col-md-3 button btn-back">Cancelar</a>
